...
clusterRole=${clusterRole:-traffic-claim-enforcer-configurator}
# The webhook name to query the current cert from is different from the service itself
webhookName=${webhookName:-traffic-claim-enforcer}
service=${service:-traffic-claim-enforcer-webhook}
secret=${secret:-traffic-claim-enforcer-webhook-certs}
...
currentCert=$(kubectl get validatingwebhookconfiguration "${webhookName}" -o jsonpath='{.webhooks[0].clientConfig.caBundle}' --ignore-not-found)
if [[ ${currentCert} != '' ]]
then
  echo "Found an existing webhook certificate.  Checking for correct ownerReferences"
  currentOwnerReference=$(kubectl get validatingwebhookconfiguration "${webhookName}" -o jsonpath='{.metadata.ownerReferences[0].kind}' --ignore-not-found)
...
echo "Creating certs in tmpdir ${tmpdir} with an expiration of ${expirationDays} days"
...
kubectl -v5 create secret generic "${secret}" \
        ...
        --from-file=cert.pem="${tmpdir}/ca.crt" \
        -n "${namespace}" \
        --dry-run -o yaml |
        kubectl apply -f -
